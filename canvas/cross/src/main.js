...
function drawCrossAt(middle, width) {
  var offset = width / 2;
  return drawCross({x: middle.x - offset, y: middle.y - offset },
                   {x: middle.x + offset, y: middle.y + offset});
}

drawCross(topLeft, bottomRight);

var addVec2d = function(v, n) { return { x: v.x + n, y: v.y + n }; };
var addVecs2d = function(v1, v2) { return { x: v1.x + v2.x, y: v1.y + v2.y }; };
